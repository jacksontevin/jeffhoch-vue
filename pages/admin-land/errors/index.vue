        ...
        <div class="tbl-card mt-3">
          ...
          >
            ...
            <template #cell(userInfo)="row">
              <span
                v-if="
                  row.item &&
                  row.item.userId &&
                  (row.item.userId.firstName || row.item.userId.lastName)
                "
                >{{
                  `${row.item.userId.firstName} ${row.item.userId.lastName}`
                }}</span
            ...
            <template #cell(actions)="row">
              <router-link :to="`/admin-land/errors/${row.item._id}`">
                <button class="btn edit mr-3">View</button>
...
import { ADMIN_FETCH_ALL_ERRORS } from '~/services/ENDPOINT'
export default {
  ...
  methods: {
    ...
    async fetchAllErrors() {
      try {
        this.loading = true
        const { data } = await ADMIN_FETCH_ALL_ERRORS(this.pagination)
        this.items = data.errors
