<template>
  <section class="admin-broker-section">
    <div class="admin-page-header">
      <h2>Broker List</h2>
      <div
        class="action-right-lg d-flex flex-column flex-md-row flex-lg-row justify-content-between align-items-center"
      >
        <div class="sidebar-search-box mb-3 order-1 order-md-2 order-lg-2">
          <b-nav-item-dropdown
            right
            class="align-self-center profile-dropdown d-inline-block order-2 order-md-1 order-lg-1"
        ...
        <div class="sidebar-search-box order-2 order-md-3 order-lg-3">
        ...
        <div class="tbl-card mt-3">
          ...
          >
            ...
            <template #cell(actions)="row">
              <router-link
                :to="`/admin-land/users/${row.item.sellerId}`"
                class="btn edit mr-3"
              >
                Edit
              </router-link>
              <router-link :to="`/admin-land/broker/${row.item.sellerId}`">
                <i class="fa fa-eye cursor-pointer text-success"></i>
              </router-link>
            </template>
          </b-table>
        </div>
...
import { ADMIN_FETCH_ALL_BROKER_LIST } from '~/services/ENDPOINT'
export default {
  name: 'AdminBroker',
  layout: 'admin',
  data() {
  ...
  methods: {
    ...
    async fetchAllUsers() {
      try {
        this.loading = true
        const { data } = await ADMIN_FETCH_ALL_BROKER_LIST(this.pagination)
